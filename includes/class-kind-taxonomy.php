...
final class Kind_Taxonomy {
	...
	public static function get_icon( $kind, $echo = false ) {
		...
		if ( file_exists( $svg ) ) {
			$return = sprintf( '<span class="svg-icon svg-%1$s" style="display: inline-block; max-width: 1rem; margin-right: 1rem;" aria-hidden="true" aria-label="%2$s" title="%2$s" >%3$s</span>', esc_attr( $kind ), esc_attr( $name ), file_get_contents( $svg ) );
		} else {
			return '';
		}
		if ( $echo ) {
